...
// 环境列表响应。
// 作用：对齐后端 `GET /workspaces/:id/environments` 返回的 `{ items, total }` 结构。
...
// 创建环境请求。
// 作用：承载 POST /workspaces/:id/environments 允许提交的字段集合。
...
// 更新环境请求。
// 作用：承载 PATCH /workspaces/:id/environments/:eid 的可选更新字段。
