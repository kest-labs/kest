...
export function useCreateDemoWorkspace() {
  ...
  return useMutation<DemoWorkspaceResult>({
    mutationFn: async () => {
      ...
      if (!workspaceId) {
        throw new Error('Workspace binding is missing for the demo workspace');
      }
    ...
    onSuccess: ({ workspace }) => {
      ...
      queryClient.invalidateQueries({ queryKey: testCaseKeys.workspace(workspace.id) });
