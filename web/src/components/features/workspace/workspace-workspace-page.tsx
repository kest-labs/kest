...
import { ApiRequestWorkbench } from '@/components/features/workspace/api-request-workbench';
...
import {
  ...
  ImportSpecsDialog,
  SpecFormDialog,
} from '@/components/features/workspace/api-spec-management-page';
import {
  buildCategoryOptions,
  findWorkspaceCategory,
  flattenWorkspaceCategories,
...
import {
  ...
  getWorkspaceWorkspaceModuleMeta,
  type WorkspaceWorkspaceModule,
...
import { useWorkspaceHistories, useWorkspaceHistory } from '@/hooks/use-histories';
import { useGenerateWorkspaceCliToken, useWorkspace } from '@/hooks/use-workspaces';
import type {
  ApiSpec,
  ApiSpecAIDraftSpec,
...
import type { CreateCategoryRequest, WorkspaceCategory } from '@/types/category';
import type {
...
const EMPTY_CATEGORIES: WorkspaceCategory[] = [];
const EMPTY_ENVIRONMENTS: WorkspaceEnvironment[] = [];
const EMPTY_HISTORIES: WorkspaceHistory[] = [];
...
const buildModuleHref = (
  ...
  const baseRoute = buildWorkspaceWorkspaceRoute(workspaceId, module);
...
export function WorkspaceWorkspacePage({
  workspaceId,
  module,
  selectedItemId,
  ...
  const workspaceName = workspaceQuery.data?.name || `${t('common.workspace')} #${workspaceId}`;

  if (!workspaceId && workspaceQuery.isLoading) {
    return (
      <div className="p-6">
        <DetailSkeleton />
      </div>
...
function WorkspaceKeysWorkspaceSection({
  workspaceId,
  workspaceName,
}: {
  workspaceId: number | string;
  ...
  const scopedWorkspaceId = String(workspace?.id ?? workspaceId);
  ...
  return (
    <WorkspaceFrame
      sidebar={
        ...
        >
          <ResourceListItem
            href={buildWorkspaceWorkspaceRoute(workspaceId, 'keys')}
            active
        ...
        >
          <div className="space-y-6">
            {workspaceQuery.isError ? (
              <Alert>
                <AlertTitle>{t('keysPage.workspaceUnavailableTitle')}</AlertTitle>
                <AlertDescription>{t('keysPage.workspaceUnavailableDescription')}</AlertDescription>
              </Alert>
            ) : null}
            ...
            <Card className="border-border-subtle">
              ...
              <CardContent>
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <DetailField label={t('keysPage.workspaceScope')}>
...
function ApiSpecsWorkspaceSection({
  ...
  const flatCategories = useMemo(() => flattenWorkspaceCategories(categories), [categories]);
  ...
  const exportFileKey =
    workspaceName
      ...
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '') || `workspace-${workspaceId}`;
  const isRefreshing =
  ...
  const handleCreateSpec = async (payload: CreateApiSpecRequest) => {
    try {
      ...
      router.replace(buildModuleHref(workspaceId, 'api-specs', createdSpec.id));
    } catch {
      // Global HTTP error handling already surfaces failure feedback.
    }
  };
  ...
  const openEditDialog = (specId: string | number) => {
    ...
    if (String(selectedItemId ?? '') !== String(specId)) {
      router.replace(buildModuleHref(workspaceId, 'api-specs', specId));
    }
  };
  ...
  const handleDeleteSpec = async () => {
    ...
    try {
      ...
      if (deletingActiveSpec) {
        router.replace(
          fallbackSpec
            ? buildModuleHref(workspaceId, 'api-specs', fallbackSpec.id)
  ...
  const queueSpecAiAction = (spec: ApiSpec, mode: 'doc' | 'test') => {
    if (String(selectedItemId ?? '') !== String(spec.id)) {
      router.replace(buildModuleHref(workspaceId, 'api-specs', spec.id));
    }
  ...
  const handleAICreateOpenChange = (open: boolean) => {
    ...
    if (!open && autoOpenAICreate) {
      router.replace(
        selectedItemId
          ? buildModuleHref(workspaceId, 'api-specs', selectedItemId)
          : buildWorkspaceApiSpecsRoute(workspaceId)
      );
    }
  };
          ...
          >
            {selectedItemId && selectedSpecQuery.isLoading ? (
              ...
              <MissingDetailState
                ...
                clearHref={buildWorkspaceApiSpecsRoute(workspaceId)}
              />
            ) : !selectedSpec ? (
              <ApiSpecsGuideState
                onOpenAICreate={() => setIsAICreateOpen(true)}
                onOpenManualCreate={() => openCreateSpecDialog()}
                testCasesHref={buildWorkspaceTestCasesRoute(workspaceId)}
              />
            ) : (
              <div className="space-y-6">
...
function EnvironmentsWorkspaceSection({
  ...
  const [duplicateTarget, setDuplicateTarget] = useState<WorkspaceEnvironment | null>(null);
  ...
  const handleEnvironmentSubmit = async (
    ...
    try {
      if (formMode === 'create') {
        ...
        setEditingEnvironmentId(null);
        setSearchQuery('');
        router.replace(buildModuleHref(workspaceId, 'environments', createdEnvironment.id));
        return;
      }
      ...
      setIsFormOpen(false);
      setEditingEnvironmentId(null);
      router.replace(buildModuleHref(workspaceId, 'environments', updatedEnvironment.id));
    } catch {
      // Global HTTP error handling already surfaces failure feedback.
    }
  };
  ...
  const handleDeleteEnvironment = async () => {
    ...
    try {
      ...
      if (isDeletingSelectedEnvironment) {
        router.replace(buildWorkspaceEnvironmentsRoute(workspaceId));
      }
    } catch {
      if (isDeletingSelectedEnvironment) {
  ...
  const handleDuplicateEnvironment = async (payload: DuplicateEnvironmentRequest) => {
    ...
    try {
      ...
      setSearchQuery('');
      router.replace(buildModuleHref(workspaceId, 'environments', duplicatedEnvironment.id));
    } catch {
      // Global HTTP error handling already surfaces failure feedback.
    }
  };
  ...
  const workspaceActionItems: ActionMenuItem[] = [
    ...
    },
    {
      ...
      icon: FileJson2,
      href: buildWorkspaceApiSpecsRoute(workspaceId),
        ...
        sidebar={
          ...
          >
            {filteredEnvironments.map(environment => (
              <ResourceListItem
                key={environment.id}
                href={buildModuleHref(workspaceId, 'environments', environment.id)}
            ...
            <div className="space-y-6">
              ...
              {isFormOpen ? (
                ...
                <MissingDetailState
                  ...
                  clearHref={buildWorkspaceEnvironmentsRoute(workspaceId)}
                />
              ) : !selectedEnvironment ? (
                  ...
                  <div className="space-y-6">
                    <Card className="border-border-subtle">
                      ...
                      <CardContent className="flex flex-wrap gap-3">
                        ...
                        <Button asChild variant="outline">
                          <Link href={buildWorkspaceTestCasesRoute(workspaceId)}>
...
function CategoriesWorkspaceSection({
  ...
  const categoriesQuery = useWorkspaceCategories({
    workspaceId: workspaceId,
    tree: true,
  });
  const selectedCategoryQuery = useWorkspaceCategory(workspaceId, selectedItemId ?? undefined);

  const flatCategories = flattenWorkspaceCategories(categoriesQuery.data?.items);
  ...
  const selectedCategoryTreeNode = findWorkspaceCategory(
    categoriesQuery.data?.items,
    selectedCategory?.id
  );
  const childCategories = selectedCategoryTreeNode?.children ?? [];
  const fullManagerHref = `${buildWorkspaceCategoriesRoute(workspaceId)}?mode=manage`;
      ...
      sidebar={
        ...
        >
          {filteredCategories.map(category => (
            <ResourceListItem
              key={category.id}
              href={buildModuleHref(workspaceId, 'categories', category.id)}
        ...
        >
          {selectedItemId && selectedCategoryQuery.isLoading ? (
            ...
            <MissingDetailState
              ...
              clearHref={buildWorkspaceCategoriesRoute(workspaceId)}
            />
          ) : !selectedCategory ? (
            <GuideState
              icon={Tags}
              ...
              actionHref={`${buildWorkspaceCategoriesRoute(workspaceId)}?mode=manage`}
...
function HistoryWorkspaceSection({
  ...
  const historiesQuery = useWorkspaceHistories({
    workspaceId: String(workspaceId),
    page: 1,
    pageSize: MAX_MODULE_ITEMS,
    entityType: entityTypeFilter !== 'all' ? entityTypeFilter : undefined,
  });
  const selectedHistoryQuery = useWorkspaceHistory(
    String(workspaceId),
        ...
        >
          {selectedItemId && selectedHistoryQuery.isLoading ? (
            ...
            <MissingDetailState
              ...
              clearHref={buildWorkspaceHistoriesRoute(workspaceId)}
            />
...
function PlaceholderWorkspaceSection({
  ...
  const t = useT('workspace');
  const moduleMeta = getWorkspaceWorkspaceModuleMeta(module);
  const moduleLabel = getWorkspaceModuleCopy(t, moduleMeta.i18nKey, 'label');
  const moduleDescription = getWorkspaceModuleCopy(t, moduleMeta.i18nKey, 'description');
      ...
      content={
        ...
        >
          <GuideState
            ...
            actionHref={isHistoryModule ? buildWorkspaceTestCasesRoute(workspaceId) : undefined}
          ...
          <BreadcrumbList className="flex-nowrap">
            <BreadcrumbItem className="min-w-0 shrink-0">
              <BreadcrumbLink asChild>
                <Link href="/workspace">{t('common.workspaces')}</Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem className="min-w-0 shrink">
              <BreadcrumbLink asChild>
                <Link href={buildWorkspaceDetailRoute(workspaceId)} className="truncate">
                  {workspaceName}
                </Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
