...
const WORKSPACE_WORKSPACE_NAV_STATE_KEY = 'kest_workspace_workspace_nav_collapsed';
...
const getInitialDesktopNavCollapsed = () => {
  ...
  return window.localStorage.getItem(WORKSPACE_WORKSPACE_NAV_STATE_KEY) === '1';
};

export function WorkspaceWorkspaceLayout({
  workspaceId,
  children,
}: {
  ...
  const updateDesktopNavCollapsed = (nextValue: boolean) => {
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.setItem(WORKSPACE_WORKSPACE_NAV_STATE_KEY, nextValue ? '1' : '0');
    }
  };
          ...
          <div className="min-w-0 flex-1 overflow-x-auto no-scrollbar">
            <nav className="flex min-w-max items-center gap-2 pr-1">
              {readyModules.map(item => {
                ...
                const href = buildWorkspaceWorkspaceRoute(workspaceId, item.value);
                ...
                return (
                  ...
                  >
                    ...
                    <span>{getWorkspaceModuleCopy(t, item.i18nKey, 'shortLabel')}</span>
                  </Link>
                );
              })}
          ...
          <div className="flex items-center justify-between gap-2 p-3">
            <Tooltip delayDuration={300}>
              ...
              <TooltipContent side="right">
                <p>{t('common.backToWorkspaceOverview')}</p>
              </TooltipContent>
            </Tooltip>
            ...
            <nav className="space-y-4">
              <div className="space-y-2">
                {readyModules.map(item => {
                  ...
                  const href = buildWorkspaceWorkspaceRoute(workspaceId, item.value);
                  const isActive = isRouteActive(href);
                  const label = getWorkspaceModuleCopy(t, item.i18nKey, 'label');
                  const navItem = (
                    <Link
                      key={item.value}
                      href={href}
