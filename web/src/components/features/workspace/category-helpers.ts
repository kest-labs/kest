import type { WorkspaceCategory } from '@/types/category';

// 带层级深度的分类节点。
// 作用：把树形分类展开后保留深度信息，便于表格缩进和排序判断。
export interface FlatWorkspaceCategory extends WorkspaceCategory {
  depth: number;
}
...
export const flattenWorkspaceCategories = (
  categories: WorkspaceCategory[] | undefined,
  depth = 0
): FlatWorkspaceCategory[] => {
  ...
  return categories.flatMap((category) => [
    { ...category, depth },
    ...flattenWorkspaceCategories(category.children, depth + 1),
  ]);
};

// 分类下拉选项构建器。
// 作用：把扁平节点转成可直接渲染在 Select 中的选项。
export const buildCategoryOptions = (categories: WorkspaceCategory[] | undefined): CategoryOption[] =>
  flattenWorkspaceCategories(categories).map((category) => ({
...
export const findWorkspaceCategory = (
  categories: WorkspaceCategory[] | undefined,
  ...
  for (const category of categories) {
    ...
    const childMatch = findWorkspaceCategory(category.children, categoryId);
...
export const collectCategoryDescendantIds = (
  ...
  const category = findWorkspaceCategory(categories, categoryId);
...
export const reorderCategoryIdsWithinSiblings = (
  categories: FlatWorkspaceCategory[],
