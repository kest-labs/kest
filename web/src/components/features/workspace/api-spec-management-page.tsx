...
import { ApiSpecAICreateDialog } from '@/components/features/workspace/api-spec-ai-create-dialog';
...
const WRITE_ROLES = WORKSPACE_MEMBER_WRITE_ROLES;
...
// 工作区平台显示文案解析器。
// 作用：把 workspace.platform 转换成适合页面展示的文本。
const getPlatformLabel = (t: WorkspaceT, platform: string) =>
  PLATFORM_LABELS[platform] || t('common.notSet');

const getDocSourceOptions = (t: WorkspaceT): Array<{ value: ApiSpecDocSource; label: string }> => [
  { value: 'manual', label: t('apiSpecsPage.docSourceManual') },
  { value: 'ai', label: t('apiSpecsPage.docSourceAi') },
];

const getLanguageOptions = (t: WorkspaceT): Array<{ value: ApiSpecLanguage; label: string }> => [
...
const getRoleLabel = (t: WorkspaceT, role?: WorkspaceMemberRole) => {
  switch (role) {
    case 'owner':
...
const getDocSourceLabel = (t: WorkspaceT, value?: ApiSpecDocSource | null) => {
  if (value === 'ai') {
...
const toHeadersRecord = (t: WorkspaceT, value: unknown) => {
...
export const downloadExportPayload = ({
  ...
  anchor.download = `${workspaceSlug || 'workspace'}-api-specs-${format}.${extension}`;
...
export function SpecFormDialog({
  ...
  const t = useT('workspace');
  const rawT = useTranslations('workspace');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="full">
        <DialogHeader>
          <DialogTitle>
            {mode === 'create'
              ? t('apiSpecs.createDialogTitle')
              : t('apiSpecsPage.editDialogTitle')}
          </DialogTitle>
          <DialogDescription>
            {mode === 'create'
        ...
        <DialogBody>
          {mode === 'edit' && isLoadingSpec ? (
            ...
            <form id="api-spec-form" className="space-y-6 py-1" onSubmit={handleSubmit}>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={value =>
                      updateDraft('categoryId', value === 'none' ? '' : value)
                    }
                  >
              ...
              {mode === 'edit' ? (
                <div className="rounded-md border border-dashed border-border-subtle bg-bg-soft p-3 text-xs text-muted-foreground">
              ...
              <div className="grid gap-4 xl:grid-cols-3">
                <div className="space-y-2">
                  <Label htmlFor="spec-doc-markdown">
                    {t('apiSpecsPage.defaultMarkdownLabel')}
                  </Label>
                  <Textarea
                ...
                <div className="space-y-2">
                  <Label htmlFor="spec-doc-markdown-zh">
                    {t('apiSpecsPage.chineseMarkdownLabel')}
                  </Label>
                  <Textarea
                ...
                <div className="space-y-2">
                  <Label htmlFor="spec-doc-markdown-en">
                    {t('apiSpecsPage.englishMarkdownLabel')}
                  </Label>
                  <Textarea
              ...
              <div className="grid gap-4 xl:grid-cols-3">
                <div className="space-y-2">
                  <Label htmlFor="spec-request-body">
        ...
        <DialogBody>
          <form id="api-spec-import-form" className="space-y-3 py-1" onSubmit={handleSubmit}>
            <Textarea
              ...
              root
            />
            <div className="text-xs text-muted-foreground">{t('apiSpecsPage.importEndpoint')}</div>
          </form>
        </DialogBody>
        <DialogFooter>
          <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>
          ...
          <div className="space-y-4 py-1">
            <div className="space-y-2">
              ...
              <Select
                value={draft.lang}
                onValueChange={value =>
                  setDraft(current => ({ ...current, lang: value as ApiSpecLanguage }))
                }
              >
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="all">{t('apiSpecsPage.batchWholeWorkspace')}</SelectItem>
                  {categories.map(category => (
                    <SelectItem key={category.value} value={category.value}>
                      {category.label}
                    </SelectItem>
            ...
            <div className="flex items-center justify-between rounded-md border border-border-subtle bg-bg-soft px-3 py-2">
              <div className="space-y-1">
                ...
                <div className="text-xs text-muted-foreground">
                  {t('apiSpecsPage.batchForceDescription')}
                </div>
              </div>
              <Switch
                id="batch-gen-force"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="sm">
        <DialogHeader>
          <DialogTitle>
            {mode === 'doc'
              ? t('apiSpecsPage.aiGenerateDocTitle')
              : t('apiSpecsPage.aiGenerateTestTitle')}
          </DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          ...
          <Button type="button" loading={isSubmitting} onClick={() => void onSubmit(draft.lang)}>
            {mode === 'doc'
              ? t('apiSpecsPage.aiGenerateDocButton')
...
export function ExampleFormDialog({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(nextErrors).length === 0) {
      ...
      try {
        requestBody = parseJsonInput(
          t,
          draft.requestBody,
          t('apiSpecsPage.exampleRequestBodyLabel')
        );
      } catch (error) {
        nextErrors.requestBody =
          error instanceof Error
        ...
        <DialogBody>
          <form id="api-example-form" className="space-y-4 py-1" onSubmit={handleSubmit}>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="api-example-status">
                    {t('apiSpecsPage.exampleResponseStatusLabel')}
                  </Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label htmlFor="api-example-duration">
                    {t('apiSpecsPage.exampleDurationLabel')}
                  </Label>
                  <Input
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              <div className="space-y-2">
                <Label htmlFor="api-example-request-headers">
                  {t('apiSpecsPage.exampleRequestHeadersLabel')}
                </Label>
                <Textarea
              ...
              <div className="space-y-2">
                <Label htmlFor="api-example-request-body">
                  {t('apiSpecsPage.exampleRequestBodyLabel')}
                </Label>
                <Textarea
              ...
              <div className="space-y-2">
                <Label htmlFor="api-example-response-body">
                  {t('apiSpecsPage.exampleResponseBodyLabel')}
                </Label>
                <Textarea
...
export function ApiSpecManagementPage({
  ...
  const [aiAction, setAiAction] = useState<{ mode: AiActionMode; spec: ApiSpec | null } | null>(
    null
  );
  ...
  const categoriesQuery = useWorkspaceApiCategories(workspaceId);
  ...
  const workspaceSlug = workspaceQuery.data?.slug ?? `workspace-${workspaceId}`;
  const workspacePlatform = workspaceQuery.data?.platform ?? '';
  ...
  const totalSpecs = specsQuery.data?.meta.total ?? workspaceStatsQuery.data?.api_spec_count ?? 0;
  ...
  const apiSpecsPath = buildApiPath(`/workspaces/${workspaceId}/api-specs`);
  const activeSpecPath = activeSpecId
    ? buildApiPath(`/workspaces/${workspaceId}/api-specs/${activeSpecId}`)
    : buildApiPath(`/workspaces/${workspaceId}/api-specs/:sid`);
  const activeSpecFullPath = activeSpecId
    ? buildApiPath(`/workspaces/${workspaceId}/api-specs/${activeSpecId}/full`)
    : buildApiPath(`/workspaces/${workspaceId}/api-specs/:sid/full`);
  const activeSpecExamplesPath = activeSpecId
    ? buildApiPath(`/workspaces/${workspaceId}/api-specs/${activeSpecId}/examples`)
    : buildApiPath(`/workspaces/${workspaceId}/api-specs/:sid/examples`);
  const activeSpecSharePath = activeSpecId
    ? buildApiPath(`/workspaces/${workspaceId}/api-specs/${activeSpecId}/share`)
    : buildApiPath(`/workspaces/${workspaceId}/api-specs/:sid/share`);
  ...
  const publishShareMutation = usePublishApiSpecShare(workspaceId);
  const deleteShareMutation = useDeleteApiSpecShare(workspaceId);
  ...
  const handleExport = async (format: ApiSpecExportFormat) => {
    try {
      ...
      downloadExportPayload({ payload, format, workspaceSlug });
  ...
  const isHeaderRefreshing =
    ...
    workspaceStatsQuery.isFetching ||
  ...
  const detailActionItems: ActionMenuItem[] = selectedSpec
    ? [
        ...
        {
          key: 'detail-workspace-overview',
          label: t('apiSpecsPage.workspaceOverview'),
          icon: FolderKanban,
          separatorBefore: true,
          href: buildWorkspaceDetailRoute(workspaceId),
        },
        {
    ...
    <>
      <main className="h-full min-h-0 overflow-y-auto">
        <div className="space-y-8 p-6 pt-6">
              ...
              <div className="space-y-3">
                ...
                >
                  <Link href={ROUTES.CONSOLE.WORKSPACES}>
                    ...
                    {t('apiSpecsPage.backToWorkspaces')}
                  </Link>
                </Button>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline">{getPlatformLabel(t, workspacePlatform)}</Badge>
                  <Badge variant="outline">
                    {t('apiSpecsPage.countSpecs', { count: totalSpecs })}
                  </Badge>
                  <Badge variant="outline">
                    {t('apiSpecsPage.countCategories', {
                      count: workspaceStatsQuery.data?.category_count ?? 0,
                    })}
                  </Badge>
                  <Badge variant="outline">
                    {t('apiSpecsPage.countMembers', {
                      count: workspaceStatsQuery.data?.member_count ?? 0,
                    })}
                  </Badge>
                </div>
              </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {workspaceQuery.isLoading || workspaceStatsQuery.isLoading || specsQuery.isLoading ? (
              <>
                <StatCardSkeleton />
                <StatCardSkeleton />
                <StatCardSkeleton />
              ...
              <>
                <StatCard
                  ...
                  value={totalSpecs}
                  description={t('apiSpecsPage.totalSpecsDescription', { slug: workspaceSlug })}
                  icon={FileJson2}
                  variant="primary"
                />
                <StatCard
                          ...
                          {specs.map(spec => (
                            <TableRow
                              ...
                              className={cn(
                                ...
                                spec.id === activeSpecId ? 'bg-bg-soft' : ''
                              )}
                              onClick={() => {
                            ...
                            >
                              ...
                              <TableCell className="min-w-[260px]">
                                <div className="space-y-1">
                                  <div className="font-mono text-xs text-muted-foreground">
                                    {t('workspacesPage.workspaceId', { id: spec.id })}
                                  </div>
                              ...
                              <TableCell>
                                {formatDate(spec.updated_at, 'YYYY-MM-DD HH:mm')}
                              </TableCell>
              ...
              <CardContent className="space-y-5 pt-6">
                {!selectedSpec ? (
                  <Alert>
                    ...
                    <AlertDescription>{t('apiSpecs.description')}</AlertDescription>
                  </Alert>
                ) : (
                  <>
                    ...
                    >
                      <TabsList className="grid w-full grid-cols-4">
                        ...
                        <TabsTrigger value="generated-test">
                          {t('apiSpecsPage.tabsGeneratedTest')}
                        </TabsTrigger>
                      </TabsList>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          <div className="rounded-md border border-border-subtle bg-bg-canvas p-4">
                            ...
                            <div className="mt-2 font-mono text-sm">
                              {selectedSpec.workspace_id ?? selectedSpec.workspace_id}
                            </div>
                          </div>
                          <div className="rounded-md border border-border-subtle bg-bg-canvas p-4">
                            <div className="text-xs uppercase tracking-[0.03125rem] text-muted-foreground">
                      ...
                      <TabsContent value="examples" className="space-y-4">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {t('apiSpecsPage.examplesDescription')}
                            </div>
                          </div>
                        ...
                        {specExamplesQuery.isLoading ? (
                          ...
                          <div className="space-y-3">
                            {specExamplesQuery.data.items.map((example: ApiSpecExample) => (
                              <div
                                key={example.id}
                                className="space-y-3 rounded-md border border-border-subtle bg-bg-canvas p-4"
                              >
                                <div className="flex flex-wrap items-center gap-2">
                                  <div className="font-medium">{example.name}</div>
                                  <Badge variant="outline">
                                    {t('apiSpecsPage.exampleStatus', {
                                      status: example.response_status,
                                    })}
                                  </Badge>
                                  <Badge variant="outline">{example.duration_ms} ms</Badge>
                                  <Badge variant="outline">
                                    {formatDate(example.created_at, 'YYYY-MM-DD HH:mm')}
                                  </Badge>
                                </div>
                          ...
                          <Alert>
                            ...
                            <AlertDescription>
                              {t('apiSpecsPage.exampleEmptyDescription')}
                            </AlertDescription>
                          </Alert>
                        )}
                      </TabsContent>
                      ...
                      <TabsContent value="generated-test" className="space-y-4">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <div className="text-sm font-medium">
                              {t('apiSpecsPage.generatedFlowTest')}
                            </div>
                            <div className="text-xs text-muted-foreground">
      ...
      <ApiSpecAICreateDialog
        ...
        onAccepted={({ specId, continueToTests }) => {
          if (continueToTests) {
            router.push(`${buildWorkspaceTestCasesRoute(workspaceId)}?fromSpec=${specId}&source=ai`);
            return;
          }
