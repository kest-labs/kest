...
import { buildWorkspaceDetailRoute, buildWorkspaceInviteRoute } from '@/constants/routes';
import {
...
import {
  WORKSPACE_MEMBER_ASSIGNABLE_ROLES,
  canEditWorkspaceMember,
  canManageWorkspaceMembers,
  canRemoveWorkspaceMember,
  sortWorkspaceMembers,
  type AssignableWorkspaceMemberRole,
  type WorkspaceMember,
  type WorkspaceMemberRole,
} from '@/types/member';
import {
  WORKSPACE_INVITATION_ASSIGNABLE_ROLES,
...
const getAssignableRole = (role?: WorkspaceMemberRole): AssignableWorkspaceMemberRole => {
...
const getInvitationStatusBadgeClassName = (status?: WorkspaceInvitationStatus) => {
  switch (status) {
    case 'active':
...
const getRoleLabel = (
  ...
  role?: WorkspaceMemberRole | WorkspaceInvitationRole
) => {
  switch (role) {
    case 'owner':
...
const getInvitationRecipientLabel = (
  t: ScopedTranslations<'workspace'>,
  invitation: Pick<WorkspaceInvitation, 'invited_user'>
) => {
...
export function WorkspaceMemberManagementPage({ workspaceId }: { workspaceId: number | string }) {
  ...
  const [roleFilter, setRoleFilter] = useState<'all' | WorkspaceMemberRole>('all');
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [candidateQuery, setCandidateQuery] = useState('');
  const [selectedCandidate, setSelectedCandidate] = useState<ApiUser | null>(null);
  const [newMemberRole, setNewMemberRole] = useState<AssignableWorkspaceMemberRole>('read');
  const [addDialogError, setAddDialogError] = useState<string | null>(null);
  const [editingMember, setEditingMember] = useState<WorkspaceMember | null>(null);
  const [editingRole, setEditingRole] = useState<AssignableWorkspaceMemberRole>('read');
  const [deleteTarget, setDeleteTarget] = useState<WorkspaceMember | null>(null);
  const [isInviteDialogOpen, setIsInviteDialogOpen] = useState(false);
  const [inviteRole, setInviteRole] = useState<WorkspaceInvitationRole>('read');
  ...
  const deleteMemberMutation = useDeleteWorkspaceMember(workspaceId);
  const invitationsQuery = useWorkspaceInvitations(
    workspaceId,
    Boolean(workspaceId) && canManageWorkspaceMembers(memberRoleQuery.data?.role)
  );
  const createInvitationMutation = useCreateWorkspaceInvitation(workspaceId);
  ...
  const roleFilterOptions: Array<{ value: 'all' | WorkspaceMemberRole; label: string }> = [
  ...
  const handleAddMember = async () => {
    if (!workspaceId) {
      setAddDialogError(t('membersPage.loadWorkspaceFirst'));
      return;
    }
  ...
  const resolveInviteLink = (invitation: WorkspaceInvitation) => {
    const invitePath = buildWorkspaceInviteRoute(invitation.slug);
  ...
  const handleCreateInvitation = async () => {
    if (!workspaceId) {
      setInviteDialogError(t('membersPage.loadWorkspaceFirst'));
      return;
    }
  ...
  const handleOpenEditDialog = (member: WorkspaceMember) => {
        ...
        <div className="space-y-6 p-4 md:p-5">
            ...
            {membersQuery.isLoading ? (
              ...
              <>
                <StatCard
                  ...
                  value={workspaceStatsQuery.data?.member_count ?? members.length}
          ...
          <Card>
            <CardHeader className="space-y-4">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  <CardTitle>{t('membersPage.workspaceMembers')}</CardTitle>
                  <CardDescription>{t('membersPage.workspaceMembersDescription')}</CardDescription>
                </div>
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <Select
                    value={roleFilter}
                    onValueChange={value => setRoleFilter(value as 'all' | WorkspaceMemberRole)}
                  >
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredMembers.map(member => {
                        ...
                        const canEdit = canEditWorkspaceMember(member, currentRole, currentUserId);
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    value={inviteRole}
                    onValueChange={value => {
                      setInviteRole(value as WorkspaceInvitationRole);
                      setInviteDialogError(null);
                    }}
                  >
                    ...
                    <SelectContent>
                      {WORKSPACE_INVITATION_ASSIGNABLE_ROLES.map(role => (
          ...
          <DialogBody>
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newMemberRole}
                  onValueChange={value => setNewMemberRole(value as AssignableWorkspaceMemberRole)}
                >
                  <SelectTrigger id="member-role">
                    <SelectValue placeholder={t('membersPage.selectRole')} />
                  </SelectTrigger>
                  <SelectContent>
                    {WORKSPACE_MEMBER_ASSIGNABLE_ROLES.map(role => (
                      <SelectItem key={role} value={role}>
                        {getRoleLabel(t, role)}
                      </SelectItem>
                    ))}
          ...
          <DialogBody>
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={editingRole}
                  onValueChange={value => setEditingRole(value as AssignableWorkspaceMemberRole)}
                >
