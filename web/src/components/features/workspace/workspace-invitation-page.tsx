...
import { ROUTES, buildWorkspaceApiSpecsRoute, buildWorkspaceInviteRoute } from '@/constants/routes';
import {
  useAcceptWorkspaceInvitation,
  useWorkspaceInvitationDetail,
  useRejectWorkspaceInvitation,
} from '@/hooks/use-workspace-invitations';
import { useT } from '@/i18n/client';
import { useAuthStore } from '@/store/auth-store';
import { isWorkspaceInvitationActive } from '@/types/workspace-invitation';
...
export function WorkspaceInvitationPage({ slug }: { slug: string }) {
  ...
  const invitationQuery = useWorkspaceInvitationDetail(slug);
  const acceptInvitationMutation = useAcceptWorkspaceInvitation(slug);
  const rejectInvitationMutation = useRejectWorkspaceInvitation(slug);

  const invitation = invitationQuery.data;
  const inviteRoute = buildWorkspaceInviteRoute(slug);
  ...
  const canRespond = isAuthenticated && isWorkspaceInvitationActive(invitation) && !wasRejected;
  const invitationName = invitation?.workspace_name || invitation?.workspace_name || '';
  const invitationSlug = invitation?.workspace_slug || invitation?.workspace_slug || '';
  ...
  const handleAcceptInvitation = async () => {
    try {
      ...
      router.replace(
        result.redirect_to ||
          (result.workspace_id ? buildWorkspaceApiSpecsRoute(result.workspace_id) : '/workspace')
      );
    } catch {
