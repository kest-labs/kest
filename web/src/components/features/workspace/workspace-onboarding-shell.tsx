...
export function WorkspaceOnboardingShell() {
  ...
  const hasCreatedFirstWorkspace = useOnboardingStore.use.hasCreatedFirstWorkspace();
    ...
    () => [
      {
        ...
        selector: '[data-onboarding="workspace-list"]',
      },
      {
        id: 'create-workspace',
        title: t('onboardingTour.steps.createWorkspaceTitle'),
        description: t('onboardingTour.steps.createWorkspaceDescription'),
        selector: '[data-onboarding="create-workspace"]',
      },
      {
        id: 'demo-card',
        title: t('onboardingTour.steps.demoTitle'),
        description: t('onboardingTour.steps.demoDescription'),
        selector: '[data-onboarding="demo-workspace-card"]',
      },
      {
        id: 'help',
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const isWorkspaceJump = normalizedKey === 'p' && event.shiftKey === false;
      ...
      if ((event.metaKey || event.ctrlKey) && (isWorkspaceJump || isApiJump)) {
        return;
      }
      ...
      if (!isTypingTarget && normalizedKey === 'g') {
        const followUpHandler = (followUpEvent: KeyboardEvent) => {
          ...
          if (followUpKey === 'p') {
            router.push(ROUTES.CONSOLE.WORKSPACES);
          } else if (followUpKey === 'a' && currentWorkspaceId) {
            router.push(buildWorkspaceRoute(currentWorkspaceId, 'api-specs'));
          }
  ...
  }, [currentWorkspaceId, markCommandPaletteHintSeen, router]);

  useEffect(() => {
    if (!hasCreatedFirstWorkspace || hasCompletedTour) {
      return;
    }
  ...
  }, [hasCompletedTour, hasCreatedFirstWorkspace]);
  ...
  const moduleCommands =
    currentWorkspaceId !== null
      ? WORKSPACE_MODULES.filter(item => item.status === 'ready').map(item => ({
          ...
          onSelect: () => {
            router.push(buildWorkspaceRoute(currentWorkspaceId, item.value));
            setIsCommandOpen(false);
          },
        }))
      : [];
  ...
  const utilityCommands = [
    {
      id: 'new-workspace',
      title: t('commandPalette.createWorkspace'),
      subtitle: t('commandPalette.createWorkspaceDescription'),
  ...
  const filteredCommands = [...utilityCommands, ...workspaceCommands, ...moduleCommands].filter(
    item => {
  ...
  return (
    <>
          ...
          <DialogBody className="space-y-4 py-2">
            ...
            <div className="rounded-xl border border-border-subtle bg-bg-soft p-4">
              ...
              <div className="mt-4 grid gap-2 md:grid-cols-2">
                ...
                <ShortcutRow label={t('helpCenter.shortcutWorkspaces')} keys="G then P" />
      ...
      <div className="sr-only">
        <Link href={ROUTES.CONSOLE.WORKSPACES}>Workspaces</Link>
        <button
          type="button"
