...
import { WORKSPACE_MEMBER_WRITE_ROLES } from '@/types/member';
import type {
...
const EMPTY_TEST_CASES: WorkspaceTestCase[] = [];
...
const parseStringRecordInput = (t: WorkspaceT, value: string, label: string) => {
...
const getRunStatusLabel = (t: WorkspaceT, status?: string) => {
...
const getTestCaseFormDraft = (testCase?: WorkspaceTestCase | null): TestCaseFormDraft => ({
...
const getDuplicateDraft = (t: WorkspaceT, testCase?: WorkspaceTestCase | null): DuplicateDraft => ({
  name: testCase ? t('testCasesPage.duplicateName', { name: testCase.name }) : '',
});

const getDefaultFromSpecName = (t: WorkspaceT, apiSpec?: ApiSpec | null) =>
  apiSpec
...
const getFromSpecDraft = (apiSpec: ApiSpec | null | undefined, t: WorkspaceT): FromSpecDraft => ({
...
function MarkdownPreview({ value, emptyLabel }: { value?: string; emptyLabel: string }) {
  ...
  return (
    <div className="markdown-content rounded-md border border-border-subtle bg-bg-soft p-5">
      <ReactMarkdown
        ...
        components={{
          ...
          pre: ({ children, ...props }) => (
            <pre
              className="overflow-x-auto rounded-md border border-border-subtle bg-bg-canvas p-4 text-xs leading-6 text-text-muted"
              {...props}
            >
              {children}
            </pre>
          ...
          td: ({ className, ...props }) => (
            <td
              className={cn('border border-border-subtle px-3 py-2 align-top', className)}
              {...props}
            />
          ),
        }}
...
function TestCaseFormDialog({
  ...
  testCase?: WorkspaceTestCase | null;
  apiSpecs: ApiSpec[];
...
export function TestCaseManagementPage({
  ...
  const [duplicateTarget, setDuplicateTarget] = useState<WorkspaceTestCase | null>(null);
  const [runTarget, setRunTarget] = useState<WorkspaceTestCase | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<WorkspaceTestCase | null>(null);
  ...
  const workspaceRole = memberRoleQuery.data?.role;
  const canWrite = workspaceRole ? WRITE_ROLES.includes(workspaceRole) : false;
  ...
  const openEditDialog = (testCase: WorkspaceTestCase) => {
    setFormMode('edit');
