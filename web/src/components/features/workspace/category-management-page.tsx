...
import {
  ActionMenu,
  type ActionMenuItem,
} from '@/components/features/workspace/action-menu';
...
} from '@/components/features/workspace/category-shared';
...
export function CategoryManagementPage({
  ...
  const flatCategories = useMemo(
    () => flattenWorkspaceCategories(categoriesQuery.data?.items),
  ...
  const selectedCategoryQuery = useWorkspaceCategory(workspaceId, activeCategoryId ?? undefined);
  const selectedCategoryFromTree = findWorkspaceCategory(categoryTree, activeCategoryId);
  const selectedCategory = selectedCategoryQuery.data ?? selectedCategoryFromTree;
  const selectedParent = findWorkspaceCategory(categoryTree, selectedCategory?.parent_id ?? null);
  const selectedChildren =
    selectedCategoryFromTree?.children ??
    flatCategories.filter((category) => category.parent_id === selectedCategory?.id);

  const editingCategory = findWorkspaceCategory(categoryTree, editingCategoryId);
  const deleteTarget = findWorkspaceCategory(categoryTree, deleteTargetId);
    ...
    <>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div className="space-y-3">
                <Button asChild variant="link" className="h-auto px-0 text-sm text-muted-foreground">
                  <Link href={buildWorkspaceDetailRoute(workspaceId)}>
                    <ArrowLeft className="h-4 w-4" />
                    {t('categoriesPage.backToWorkspaceOverview')}
                  </Link>
                </Button>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline">
                    {t('categoriesPage.countCategories', {
                      count: workspaceStatsQuery.data?.category_count ?? totalCategories,
                    })}
                  </Badge>
                  <Badge variant="outline">
                    {t('categoriesPage.countSpecs', {
                      count: workspaceStatsQuery.data?.api_spec_count ?? 0,
                    })}
                  </Badge>
                  <Badge variant="outline">
      ...
      {isForbidden ? (
        <Alert variant="destructive">
          <AlertTitle>{t('categoriesPage.noWorkspaceAccessTitle')}</AlertTitle>
          <AlertDescription>
            {t('categoriesPage.noWorkspaceAccessDescription')}
          </AlertDescription>
        </Alert>
      ) : null}
