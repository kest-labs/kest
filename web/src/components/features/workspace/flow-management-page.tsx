...
type WorkspaceTranslator = ScopedTranslations<'workspace'>;
...
const getStatusLabel = (t: WorkspaceTranslator, status: FlowRunStatus | 'idle') => {
  switch (status) {
    case 'passed':
...
const getCanvasNodeLabel = (t: WorkspaceTranslator, node: FlowCanvasNode, index?: number) => {
          ...
          <CardContent>
            <Tabs defaultValue="request" className="space-y-4">
              ...
              <TabsContent value="request" className="space-y-4">
                ...
                <details className="rounded-xl border border-border-subtle bg-bg-soft p-4">
                    ...
                    <div className="space-y-2">
                      {captureDefinitions.map(capture => (
                        <div
                          ...
                          className="rounded-xl border border-border-subtle bg-bg-soft p-3"
                        >
...
function ResultField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-border-subtle bg-bg-soft p-4">
      <p className="text-xs font-medium uppercase tracking-[0.03125rem] text-text-muted">{label}</p>
...
export function WorkspaceFlowManagementPage({
  workspaceId,
  selectedItemId,
}: {
  ...
  const workspaceName = workspaceQuery.data?.name || t('flowPage.workspaceFallback', { id: workspaceId });
  ...
  const flowHistoryHref = `${buildWorkspaceHistoriesRoute(workspaceId)}?entityType=${FLOW_HISTORY_ENTITY_TYPE}`;
  ...
  const navigateToFlow = (flowId: string | null) => {
    ...
    setLiveStepResults({});
    setValidationState(createEmptyValidationState());
    const baseHref = buildWorkspaceFlowsRoute(workspaceId);
  ...
  const finalizeRun = async (runId: string) => {
    ...
    const run = await flowService.getRun(workspaceId, selectedFlowId, runId);
    if (run.step_results) {
  ...
  const streamRun = async (runId: string, baseUrl?: string) => {
    ...
    try {
      await flowService.streamRun(workspaceId, selectedFlowId, runId, {
  ...
  const editorContent = selectedFlowQuery.isLoading ? (
    ...
    <Card className="rounded-xl border-dashed border-border-subtle bg-bg-canvas">
      <CardContent className="space-y-4 py-14 text-center">
        ...
        <Button asChild variant="outline">
          <Link href={buildWorkspaceFlowsRoute(workspaceId)}>{t('common.clearSelection')}</Link>
        </Button>
      </CardContent>
    </Card>
  ) : !selectedFlowId ? (
    ...
    <div className="flex flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border-subtle bg-bg-soft px-4 py-4 md:px-6">
        <Select
          value={
            selectedRunEnvironmentId === null
      ...
      <div className="flex flex-col xl:min-h-0 xl:flex-1 xl:flex-row">
        <div className="min-w-0 flex-1 bg-bg-soft">
          <div className="min-h-[720px] p-4 md:p-6">
            <div className="h-[min(72vh,960px)] min-h-[720px] overflow-hidden rounded-xl border border-border-subtle bg-bg-canvas">
              <ReactFlow
                nodes={nodes}
                edges={edges}
                nodeTypes={nodeTypes}
        ...
        {isDesktopInspectorCollapsed ? (
          ...
          <aside className="hidden w-[380px] shrink-0 border-l border-border-subtle bg-bg-soft xl:block">
